...
logger = logging.getLogger("masq_project_setup")


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Setup MASQ project directory"
    )

    parser.add_argument(
        "--working-dir", "-d",
        help="Working directory to setup project in. "
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if not os.path.exists(working_dir):
        logger.error(
            "Working directory %s does not exist", working_dir)
        raise ValueError(f"Working directory {working_dir} does not exist")
    if not os.path.isdir(working_dir):
        logger.error(
            "Specified working directory %s is not a directory",
            working_dir)
        raise ValueError(f"Working directory {working_dir} is not a directory")

    _create_template_config_file(working_dir)
    _create_snakefile(working_dir)


def _create_template_config_file(working_dir: str) -> None:
    filename = os.path.join(working_dir, "config.yaml")
    if os.path.exists(filename):
        logger.error("Config file %s already exists", filename)
        raise ValueError(f"Config file {filename} already exists")
    with open(filename, "w") as f:
        f.write(CONFIG_TEMPLATE)


def _create_snakefile(working_dir: str) -> None:
    filename = os.path.join(working_dir, "Snakefile")
    if os.path.exists(filename):
        logger.error("Snakefile %s already exists", filename)
        raise ValueError(f"Snakefile {filename} already exists")
    with open(filename, "w") as f:
        f.write(SNAKEFILE_TEMPLATE)


CONFIG_TEMPLATE = """
# Config file example template for MASQ pipeline

# Sample information
# Group name for samples: Replace with actual group name
groupname: "<group name>"

# One sample, or list of samples. Replace with actual sample names
samples: 
- "<sample 1 name>"
- "<sample 2 name>"
...
# Locus info table. See examples for precise format.
# Replace with actual file name
SNV_table: "<example_inputs/locus_table.MASQ.txt>"

# (Optional) Bad loci 
# Loci that should be dropped from analysis for reasons not detected
# by automatic QC. Replace with actual list if needed
# bad_loci: [5,8]

# FASTQs
# Expects r1.fastq.gz and r2.fastq.gz in folder.
# Replace with actual paths for each sample
fastq_dirs: 
  <sample 1 name>: "<sample 1 fastq dir>"
  <sample 2 name>: "<sample 2 fastq dir>"

# Whole genome or other BAM file to use to identify non-target variants
# WGS_bam and WGS_name can be lists for multiple BAMs
# BAM must be indexed
# Replace with actual paths to BAM files for each sample
WGS_bam: 
- "<sample 1 WGS BAM (ex: example_inputs/example_MASQ.normal.wgs.bam)>"
- "<sample 2 WGS BAM (ex: example_inputs/example_MASQ.tumor.wgs.bam)>"

WGS_name: 
- "<sample 1 wgs name (ex: normal_wgs_bam)>"
- "<sample 2 wgs name (ex: tumor_wgs_bam)"
...
# Replace with actual path to reference genome FASTA file
ref_genome_fa: "<path to reference genome FASTA file>"
